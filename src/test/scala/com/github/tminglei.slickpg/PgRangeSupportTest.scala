...
class PgRangeSupportTest {
  ...
  class RangeTestTable(tag: Tag) extends Table[RangeBean](tag, /*Some("test"),*/ "RangeTest") {
    ...
    def * = (id, intRange, floatRange, tsRange) <> (RangeBean.tupled, RangeBean.unapply _)
  }

  val RangeQuery = TableQuery[RangeTestTable]
  ...
  def testSimpleInsertFetch(): Unit = {
    db withSession { implicit session: Session =>
      ...
      val rec1 = RangeQuery.where(_.id === testRec1.id.bind).map(t => t).first()
  ...
  def testRangeFunctions(): Unit = {
    db withSession { implicit session: Session =>
      ...
      val q0 = RangeQuery.where(_.tsRange @>^ timestamp("2011-10-01 15:30:00").bind).sortBy(_.id).map(t => t)
      println(s"'@>^' sql = ${q0.selectStatement}")
      assertEquals(List(testRec2), q0.list())

      val q01 = RangeQuery.where(timestamp("2011-10-01 15:30:00").bind <@^: _.tsRange).sortBy(_.id).map(t => t)
      println(s"'<@^' sql = ${q01.selectStatement}")
      assertEquals(List(testRec2), q01.list())

      val q1 = RangeQuery.where(_.floatRange @> Range(10.5f, 12f).bind).sortBy(_.id).map(t => t)
      println(s"'@>' sql = ${q1.selectStatement}")
      assertEquals(List(testRec3), q1.list())

      val q11 = RangeQuery.where(Range(10.5f, 12f).bind <@: _.floatRange).sortBy(_.id).map(t => t)
      println(s"'<@' sql = ${q11.selectStatement}")
      assertEquals(List(testRec3), q11.list())

      val q2 = RangeQuery.where(_.intRange @& Range(4,6).bind).sortBy(_.id).map(t => t)
      println(s"'@&' sql = ${q2.selectStatement}")
      assertEquals(List(testRec1, testRec3), q2.list())

      val q3 = RangeQuery.where(_.intRange << Range(10, 15).bind).sortBy(_.id).map(t => t)
      println(s"'<<' sql = ${q3.selectStatement}")
      assertEquals(List(testRec1, testRec3), q3.list())

      val q4 = RangeQuery.where(_.intRange >> Range(10, 15).bind).sortBy(_.id).map(t => t)
      println(s"'<<' sql = ${q4.selectStatement}")
      assertEquals(List(testRec2), q4.list())

      val q5 = RangeQuery.where(_.floatRange &< Range(2.9f, 7.7f).bind).sortBy(_.id).map(t => t)
      println(s"'&<' sql = ${q5.selectStatement}")
      assertEquals(List(testRec1), q5.list())

      val q6 = RangeQuery.where(_.floatRange &> Range(2.9f, 7.7f).bind).sortBy(_.id).map(t => t)
      println(s"'&>' sql = ${q6.selectStatement}")
      assertEquals(List(testRec2, testRec3), q6.list())

      val q7 = RangeQuery.where(_.intRange -|- Range(5, 31).bind).sortBy(_.id).map(t => t)
      println(s"'-|-' sql = ${q7.selectStatement}")
      assertEquals(List(testRec1, testRec2, testRec3), q7.list())

      val q8 = RangeQuery.where(_.id === 41L).map(t => t.intRange + Range(3, 6).bind)
      println(s"'+' sql = ${q8.selectStatement}")
      assertEquals(Range(1, 6), q8.first())

      val q9 = RangeQuery.where(_.id === 41L).map(t => t.intRange * Range(3, 6).bind)
      println(s"'*' sql = ${q9.selectStatement}")
      assertEquals(Range(3, 5), q9.first())

      val q10 = RangeQuery.where(_.id === 41L).map(t => t.intRange - Range(3, 6).bind)
